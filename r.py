# Regenerate sentences with more natural, contextual usage.
# Heuristics choose patterns by part-of-speech or function type.
# Output JSON keeps the same structure and provides two example sentences per word,
# each with full-sentence pinyin (tone marks) using fixed templates plus the word's pinyin inserted.
...
question_words = {"什么","谁","哪儿","哪里","哪","怎么","为什么","多少","几","多久","多远","多大","哪儿","哪边"}
...
# common adjective cues in English gloss
...
# common place cues (english)
place_keywords = {"city","country","school","university","park","hospital","restaurant","supermarket","airport","station","home","family","company","office","library","bank","hotel","store","shop"}

# verb cues: english starting with 'to '
def guess_pos(hanzi, english):
    e = (english or "").strip().lower()
    if hanzi in particles:
        return "particle"
    if hanzi in question_words:
        return "question"
    if hanzi in pronouns:
        return "pronoun"
    if hanzi in measure_words or "measure" in e or "classifier" in e:
        return "measure"
    if hanzi in prepositions or "preposition" in e:
        return "preposition"
    if hanzi in adverbs or "adverb" in e:
        return "adverb"
    if hanzi in numbers or re.fullmatch(r"\d+", hanzi):
        return "number"
    if hanzi in time_words or any(k in e for k in ["today","tomorrow","yesterday","morning","evening","year","month","week","day","now"]):
        return "time"
    if e.startswith("to ") or "(v" in e or "verb" in e:
        return "verb"
    if any(k in e for k in adj_keywords) or "(adj" in e or "adjective" in e:
        return "adjective"
    # rough noun guess
    return "noun"

# --- Sentence generators per type ---

def gen_particle(hanzi, pinyin, english, hsk):
    if hanzi == "吗":
        return [
            {"chinese":"你现在忙吗？","pinyin":"Nǐ xiànzài máng ma?","english":"Are you busy now?"},
    ...
    if hanzi == "吧":
        return [
            {"chinese":"太晚了，我们回家吧。","pinyin":"Tài wǎn le, wǒmen huíjiā ba.","english":"It's too late; let's go home."},
            {"chinese":"休息一下吧。","pinyin":"Xiūxi yíxià ba.","english":"Take a short break."}
        ]
    if hanzi == "了":
        return [
            {"chinese":"我吃饭了。","pinyin":"Wǒ chīfàn le.","english":"I ate (already)."},
            {"chinese":"雨停了，我们出发吧。","pinyin":"Yǔ tíng le, wǒmen chūfā ba.","english":"The rain has stopped; let's set off."}
        ]
    if hanzi == "过":
        return [
            {"chinese":"我去过北京。","pinyin":"Wǒ qù guo Běijīng.","english":"I have been to Beijing."},
            {"chinese":"你吃过四川菜吗？","pinyin":"Nǐ chī guo Sìchuān cài ma?","english":"Have you eaten Sichuan food before?"}
        ]
    if hanzi == "着":
        return [
            {"chinese":"门开着呢。","pinyin":"Mén kāizhe ne.","english":"The door is open."},
    ...
    if hanzi == "得":
        return [
            {"chinese":"他说得很快。","pinyin":"Tā shuō de hěn kuài.","english":"He speaks very fast."},
            {"chinese":"你做得不错。","pinyin":"Nǐ zuò de búcuò.","english":"You did quite well."}
        ]
    if hanzi == "地":
        return [
            {"chinese":"他认真地做作业。","pinyin":"Tā rènzhēn de zuò zuòyè.","english":"He does homework carefully."},
            {"chinese":"她高兴地唱起歌来。","pinyin":"Tā gāoxìng de chàng qǐ gē lái.","english":"She started to sing happily."}
        ]
    if hanzi == "呢":
        return [
            {"chinese":"我在看书呢。","pinyin":"Wǒ zài kànshū ne.","english":"I'm reading (right now)."},
            {"chinese":"我说完了，你呢？","pinyin":"Wǒ shuō wán le, nǐ ne?","english":"I'm finished speaking. How about you?"}
        ]
    # generic fallback: show as modal particle
    return [
        {"chinese":f"问题不大{hanzi}。","pinyin":f"Wèntí bú dà {pinyin}.","english":"It's not a big problem (softening tone)."},
        {"chinese":f"我们走{hanzi}。","pinyin":f"Wǒmen zǒu {pinyin}.","english":"Let's go (soft suggestion)."},
    ]

def gen_measure(hanzi, pinyin, english, hsk):
    # choose typical nouns per measure
    choices = {
        ...
        "次": [("一次机会","Yí cì jīhuì","one chance"),("去过两次","Qù guo liǎng cì","have been there twice")],
        "双": [("一双鞋","Yì shuāng xié","a pair of shoes"),("一双筷子","Yì shuāng kuàizi","a pair of chopsticks")]
    }
    if hanzi in choices:
        (cn1, py1, en1), (cn2, py2, en2) = choices[hanzi]
        return [
            {"chinese":cn1,"pinyin":py1,"english":en1},
            {"chinese":cn2,"pinyin":py2,"english":en2}
        ]
    # generic with 个
    return [
        {"chinese":"一个苹果。","pinyin":"Yí gè píngguǒ.","english":"One apple."},
...
def gen_pronoun(hanzi, pinyin, english, hsk):
    if hanzi=="我":
        return [
            {"chinese":"我喜欢学习中文。","pinyin":"Wǒ xǐhuān xuéxí Zhōngwén.","english":"I like studying Chinese."},
            {"chinese":"周末我常常去公园。","pinyin":"Zhōumò wǒ chángcháng qù gōngyuán.","english":"On weekends I often go to the park."}
        ]
    if hanzi=="你":
        return [
            {"chinese":"你今天怎么样？","pinyin":"Nǐ jīntiān zěnmeyàng?","english":"How are you today?"},
            {"chinese":"这本书给你。","pinyin":"Zhè běn shū gěi nǐ.","english":"This book is for you."}
        ]
    # generic
    return [
        {"chinese":f"{hanzi}在听课。","pinyin":f"{pinyin} zài tīng kè.","english":f"{english.capitalize()} is attending class."},
...
def gen_question(hanzi, pinyin, english, hsk):
    ...
    if hanzi in mapping:
        s1 = mapping[hanzi]
        return [
            {"chinese":s1[0],"pinyin":s1[1],"english":s1[2]},
            {"chinese":f"{hanzi}时候出发最合适？","pinyin":f"{pinyin} shíhou chūfā zuì héshì?","english":f"When/which/what is best to set off?"}
        ]
    return [
        {"chinese":f"{hanzi}时候我们见面？","pinyin":f"{pinyin} shíhou wǒmen jiànmiàn?","english":"When shall we meet?"},
        {"chinese":"你想去哪儿？","pinyin":"Nǐ xiǎng qù nǎr?","english":"Where do you want to go?"}
    ]

def gen_preposition(hanzi, pinyin, english, hsk):
    mapping = {
        "在": ("我在家工作。","Wǒ zài jiā gōngzuò.","I work at home."),
        "对": ("我对历史很感兴趣。","Wǒ duì lìshǐ hěn gǎn xìngqù.","I'm very interested in history."),
        "给": ("请给我一杯水。","Qǐng gěi wǒ yì bēi shuǐ.","Please give me a glass of water."),
        "从": ("我从上海飞到北京。","Wǒ cóng Shànghǎi fēi dào Běijīng.","I flew from Shanghai to Beijing."),
        "到": ("我们到学校去。","Wǒmen dào xuéxiào qù.","We're going to the school."),
        "跟": ("我跟他一起学习。","Wǒ gēn tā yìqǐ xuéxí.","I study together with him."),
        "向": ("他向老师请教问题。","Tā xiàng lǎoshī qǐngjiào wèntí.","He asks the teacher for guidance."),
        "往": ("往左走，就到了。","Wǎng zuǒ zǒu, jiù dào le.","Go left and you'll be there."),
        "把": ("请把门关上。","Qǐng bǎ mén guān shàng.","Please close the door."),
        "被": ("他被雨淋湿了。","Tā bèi yǔ lín shī le.","He was soaked by the rain."),
        "比": ("今天比昨天冷。","Jīntiān bǐ zuótiān lěng.","Today is colder than yesterday."),
        "离": ("学校离我家不远。","Xuéxiào lí wǒ jiā bù yuǎn.","The school isn't far from my home.")
    }
    if hanzi in mapping:
        s1 = mapping[hanzi]
        s2 = {
            "在": ("她在图书馆看书。","Tā zài túshūguǎn kànshū.","She reads at the library."),
            "对": ("这个决定对大家都有好处。","Zhège juédìng duì dàjiā dōu yǒu hǎochù.","This decision benefits everyone."),
            "给": ("医生给了我一些建议。","Yīshēng gěi le wǒ yìxiē jiànyì.","The doctor gave me some advice."),
            "从": ("他从小就喜欢音乐。","Tā cóng xiǎo jiù xǐhuān yīnyuè.","He has liked music since childhood."),
            "到": ("火车几点到？","Huǒchē jǐ diǎn dào?","What time does the train arrive?"),
            "跟": ("跟我来。","Gēn wǒ lái.","Come with me."),
            "向": ("请向右看。","Qǐng xiàng yòu kàn.","Please look to the right."),
            "往": ("往前走三分钟就到了。","Wǎng qián zǒu sān fēnzhōng jiù dào le.","Walk forward three minutes and you'll arrive."),
            "把": ("我把作业放在桌子上了。","Wǒ bǎ zuòyè fàng zài zhuōzi shàng le.","I put the homework on the table."),
            "被": ("窗户被风吹开了。","Chuānghu bèi fēng chuī kāi le.","The window was blown open by the wind."),
            "比": ("他比我高一点。","Tā bǐ wǒ gāo yìdiǎn.","He's a bit taller than me."),
            "离": ("银行离地铁站很近。","Yínháng lí dìtiězhàn hěn jìn.","The bank is very close to the subway station.")
        }[hanzi]
        return [
            {"chinese":s1[0],"pinyin":s1[1],"english":s1[2]},
            {"chinese":s2[0],"pinyin":s2[1],"english":s2[2]}
        ]
    # generic
    return [
        {"chinese":"他在教室里学习。","pinyin":"Tā zài jiàoshì lǐ xuéxí.","english":"He studies in the classroom."},
...
def gen_adverb(hanzi, pinyin, english, hsk):
    ...
    # fallback
    return [
        {"chinese":f"{hanzi}好。","pinyin":f"{pinyin} hǎo.","english":f"{english.capitalize()} is fine."},
        {"chinese":f"他{hanzi}来了。","pinyin":f"Tā {pinyin} lái le.","english":f"He {english} came."}
    ]
...
def gen_time(hanzi, pinyin, english, hsk):
    ...
    if hanzi in mapping:
        return [{"chinese":a,"pinyin":b,"english":c} for a,b,c in mapping[hanzi]]
    return [
        {"chinese":f"{hanzi}我们再联系。","pinyin":f"{pinyin} wǒmen zài liánxì.","english":"Let's be in touch then."},
        {"chinese":f"{hanzi}我要出门。","pinyin":f"{pinyin} wǒ yào chūmén.","english":"I'm going out then."}
    ]

def gen_number(hanzi, pinyin, english, hsk):
    return [
        {"chinese":f"{hanzi}个人。","pinyin":f"{pinyin} gè rén.","english":f"{english.capitalize()} people."},
        {"chinese":f"{hanzi}杯水。","pinyin":f"{pinyin} bēi shuǐ.","english":f"{english.capitalize()} cups of water."}
    ]

def gen_adjective(hanzi, pinyin, english, hsk):
    # special mapping for a few very common adjectives
    special = {
    ...
    # general patterns: 很 + adj ; 比较句
    return [
        {"chinese":f"这个地方很{hanzi}。","pinyin":f"Zhège dìfāng hěn {pinyin}.","english":f"This place is very {english}."},
        {"chinese":f"这件事比我想的更{hanzi}。","pinyin":f"Zhè jiàn shì bǐ wǒ xiǎng de gèng {pinyin}.","english":f"This matter is even more {english} than I thought."}
    ]

def gen_verb(hanzi, pinyin, english, hsk):
    # verb-specific collocations for a few very common verbs
    vmap = {
    ...
    # generic verb frames
    return [
        {"chinese":f"我常常{hanzi}。","pinyin":f"Wǒ chángcháng {pinyin}.","english":f"I often {english}." if english else "I often do this."},
...
def gen_noun(hanzi, pinyin, english, hsk):
    ...
    # general noun frames
    return [
        {"chinese":f"这是{hanzi}。","pinyin":f"Zhè shì {pinyin}.","english":f"This is {english}." if english else "This is it."},
...
def generate_examples(hanzi, pinyin, english, hsk):
    ...
    if pos == "particle":
        return gen_particle(hanzi, pinyin, english, hsk)
    if pos == "measure":
        return gen_measure(hanzi, pinyin, english, hsk)
    if pos == "pronoun":
        return gen_pronoun(hanzi, pinyin, english, hsk)
    if pos == "question":
        return gen_question(hanzi, pinyin, english, hsk)
    if pos == "preposition":
        return gen_preposition(hanzi, pinyin, english, hsk)
    if pos == "adverb":
        return gen_adverb(hanzi, pinyin, english, hsk)
    if pos == "time":
        return gen_time(hanzi, pinyin, english, hsk)
    if pos == "number":
        return gen_number(hanzi, pinyin, english, hsk)
    if pos == "adjective":
        return gen_adjective(hanzi, pinyin, english, hsk)
    if pos == "verb":
        return gen_verb(hanzi, pinyin, english, hsk)
    # noun or unknown
